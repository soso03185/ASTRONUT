...
void Model::Load()
{
	...
	HR_T(CommonApp::m_pInstance->GetDevice()->CreateBuffer(&BMBDesc, nullptr, m_pBoneMatrixConstantBuffer.GetAddressOf()));
}
...
void Model::ParsingNode(aiNode* pNode, Node* pParentNode, const aiScene* pScene)
{
	...
	Node* newNode = new Node{ pNode->mName.C_Str(), this, Matrix(&pNode->mTransformation.a1), meshes };
...
Mesh* Model::ParsingMesh(aiMesh* mesh, const aiScene* pScene)
{
	...
	return new Mesh{ vertices, indices, textures, baseColor };
}
...
void Model::ProcessBoneInfo(aiMesh* mesh, std::vector<Vertex>& vertices)
{
	...
	for (UINT i = 0; i < mesh->mNumBones; ++i)
	{
		...
		if (m_boneMap.find(boneName) == m_boneMap.end())
		{
			...
			m_bones[m_boneIndex].offsetMatrix = Matrix(&bone->mOffsetMatrix.a1);
		}
		else
		{
...
void Model::UpdateBoneMatrix()
{
	...
	CommonApp::m_pInstance->GetDevcon()->VSSetConstantBuffers(4, 1, m_pBoneMatrixConstantBuffer.GetAddressOf());
	CommonApp::m_pInstance->GetDevcon()->UpdateSubresource(m_pBoneMatrixConstantBuffer.Get(), 0, nullptr, &BoneMatrixCB, 0, 0);
}
	...
	const aiScene* scene)
{
	...
	for (UINT i = 0; i < material->GetTextureCount(type); ++i)
	{
		...
		if (!skip)
		{
			...
			HR_T(CreateWICTextureFromFile(CommonApp::m_pInstance->GetDevice().Get(), CommonApp::m_pInstance->GetDevcon().Get(), fileNameWs.c_str(), nullptr, &texture.Source));
...
ID3D11ShaderResourceView* Model::LoadEmbeddedTexture(const aiTexture* embeddedTexture)
{
	...
	if (embeddedTexture->mHeight)
	{
		...
		HR_T(CommonApp::m_pInstance->GetDevice()->CreateTexture2D(&desc, &subResourceData, &texture2D));
		HR_T(CommonApp::m_pInstance->GetDevice()->CreateShaderResourceView(texture2D, nullptr, &texture));

		return texture;
	}

	const size_t size = embeddedTexture->mWidth;
	HR_T(CreateWICTextureFromMemory(CommonApp::m_pInstance->GetDevice().Get(), CommonApp::m_pInstance->GetDevcon().Get(), reinterpret_cast<const unsigned char*>(embeddedTexture->pcData), size, nullptr, &texture));
	return texture;
}
