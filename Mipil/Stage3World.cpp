...
void Stage3World::Initialize()
{
	LOG_CONTENTS("Stage3 Init");
	...
	std::shared_ptr<BlockGenerator> blockGenerator = std::make_shared<BlockGenerator>(shared_from_this(), "../Resources/Map/stage_3.yaml");
	std::shared_ptr<PartGenerator> partGenerator = std::make_shared<PartGenerator>(shared_from_this(), "../Resources/Map/stage_3.yaml");
	...
	SetPlayersPosition({ 90.f, 90.f, 810.f }, { 90.f, 90.f, 90.f });
	...
	MiniMapUI->SetTexturePath(L"../Resources/Textures/4.png");
	...
	// 스테이지 클리어 패널 로켓 이미지(스테이지 3은 60_2)
	...
	rocketProcessPercent.lock()->SetText(L"60%");
...
void Stage3World::Update(float deltaTime)
{
...
void Stage3World::OnEnter()
{
	PuzzleWorld::OnEnter();
}

void Stage3World::OnExit()
{
	PuzzleWorld::OnExit();
}
