...
void Stage5World::Initialize()
{
	LOG_CONTENTS("Stage5 Init");
	...
	std::shared_ptr<BlockGenerator> blockGenerator = std::make_shared<BlockGenerator>(shared_from_this(), "../Resources/Map/stage_5.yaml");
	std::shared_ptr<PartGenerator> partGenerator = std::make_shared<PartGenerator>(shared_from_this(), "../Resources/Map/stage_5.yaml");
	...
	SetPlayersPosition({ 270.f, 270.f, 90.f }, { 630.f, 270.f, 90.f });
	...
	SetDivisionMapPosition({ 450.f, 0.f, 540.f });
	...
	hollowBoxRigidBody.lock()->CreateHollowBox(900.f, 720.f, 1080.f, { 0.f, 0.f, 0.f });

	// 미니맵 UI(홀로그램 패널 자식)
	auto MiniMapUI = m_UIManager->CreateUI<UIPanel>(L"Hologram_MiniMapUI");
	MiniMapUI->SetTexturePath(L"../Resources/Textures/3.png");
	MiniMapUI->SetPosition({ 479.f, 113.f });
	MiniMapUI->SetSize({ 413.f, 494.f });
	...
	floorBlock->SetPosition({ 450.f, -60.f, 540.f });		// 바닥 블록 위치 설정
	...
	// 스테이지 클리어 패널 로켓 이미지(스테이지 5은 100_2)
	auto stageClear_Rocket = m_UIManager->GetUIObject<UIPanel>(L"StageClear_Rocket");
	stageClear_Rocket.lock()->GetUIInstance().lock()->LoadTextureResource(L"../Resources/Textures/100_2.png");

	// 스테이지 클리어 패널 퍼센트(스테이지 5은 100%)
	auto rocketProcessPercent = m_UIManager->GetUIObject<UIText>(L"StageClear_RocketProcessPercent");
	rocketProcessPercent.lock()->SetText(L"100%");

	/// GridManager
	m_gridManager = std::make_shared<GridManager>();
	m_gridManager->CreateMap(5, 4, 6, 0.f, 0.f, 0.f, 180.f);
...
void Stage5World::Update(float deltaTime)
{
...
void Stage5World::OnEnter()
{
	PuzzleWorld::OnEnter();
}

void Stage5World::OnExit()
{
	PuzzleWorld::OnExit();
}
