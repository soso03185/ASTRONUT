...
void Stage4World::Initialize()
{
	LOG_CONTENTS("Stage4 Init");
	...
	std::shared_ptr<BlockGenerator> blockGenerator = std::make_shared<BlockGenerator>(shared_from_this(), "../Resources/Map/stage_4.yaml");
	std::shared_ptr<PartGenerator> partGenerator = std::make_shared<PartGenerator>(shared_from_this(), "../Resources/Map/stage_4.yaml");
	...
	SetPlayersPosition({ 270.f, 90.f, 90.f }, { 630.f, 90.f, 90.f });
	...
	MiniMapUI->SetTexturePath(L"../Resources/Textures/2.png");
	...
	// 스테이지 클리어 패널 로켓 이미지(스테이지 4은 81_2)
	auto stageClear_Rocket = m_UIManager->GetUIObject<UIPanel>(L"StageClear_Rocket");
	stageClear_Rocket.lock()->GetUIInstance().lock()->LoadTextureResource(L"../Resources/Textures/60_2.png");

	// 스테이지 클리어 패널 퍼센트(스테이지 4은 81%)
	auto rocketProcessPercent = m_UIManager->GetUIObject<UIText>(L"StageClear_RocketProcessPercent");
	rocketProcessPercent.lock()->SetText(L"81%");
...
void Stage4World::Update(float deltaTime)
{
...
void Stage4World::OnEnter()
{
	PuzzleWorld::OnEnter();
}

void Stage4World::OnExit()
{
	PuzzleWorld::OnExit();
}
